...
class Player_1():
    def __init__(self, x, y, player_size, sprite_sheet, animation_frames):
        ...
        self.action = 0  #0: Idle #1: Block #2: Punch #3 Jump #4 Dead
        self.frame_index = 0
    ...
    def load_images(self, sprite_sheet, animation_frames):
        ...
        for y, animation in enumerate(animation_frames):  
            ...
            for x in range(animation):
                ...
                temp_img_list.append(pygame.transform.scale(temp_img, (145, 195))) 
    ...
    def move(self, surface, target):
        ...
        #Blocking
        if keys[pygame.K_q] and self.stamina >= 0 and self.block_cd == 0 and self.alive == True and target.health > 0:
            self.stamina -= 2
            self.block = True
            self.rect.width = 120  
        else:
        ...
        #Attacking
        if keys[pygame.K_e] and self.block == False and self.alive == True and target.health > 0:
    ...
    def attack(self, surface, target):
        if self.attack_cd == 0:
            ...
            if self.flip == True:
                attack_rect = pygame.Rect(self.rect.left - 28, self.rect.y - 2, self.rect.width / 2.7, self.rect.height)   
            else:
    ...
    def draw(self, surface):
        ...
        if self.flip == True and self.block == True:
            surface.blit(img, (self.rect.x - 25, self.rect.y))
        elif self.flip == True:
            surface.blit(img, (self.rect.x - 65, self.rect.y))
        else:
